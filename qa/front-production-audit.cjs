...
const env = fs.existsSync(localEnv) ? Object.fromEntries(fs.readFileSync(localEnv, 'utf8').split(/\r?\n/).filter(x => x && !x.startsWith('#') && x.includes('=')).map(x => { const i=x.indexOf('=');return [x.slice(0,i),x.slice(i+1).replace(/^['"]|['"]$/g,'')]; })) : process.env;
